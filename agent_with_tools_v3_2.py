"""
Task-performing agent -- v3.2 (bug-fix pass on v3.1, no new functionality).
v3.1 testing found four real bugs, addressed here:

    1. Adding sqrt/abs/round/pow support to the calculator tool.
    2. For notes the memory-related tool descriptions were shortened, since the more
       wordy v3.1 descriptions may have made the selection worse,
       and add_note now structures inputs better.
    3. Forget function was too strict. Now it should recognize facts from memory easier.
    4. Stuck loop fix: after a false "no match,"
       the model sometimes asked for yes/no confirmation when there was no 
       entry to be deleted. Forget's no-match message now explicitly states
       nothing was staged and confirmation shouldn't be requested.

Folder layout expected next to this script:
    agent_with_tools_v3_2.py
